...
fn without_the_entity_latch_a_concurrent_set_loses_a_property() {
    ...
    panic!(
        "three runs with the entity latches off lost nothing — the latch is not the \
...
/// increments, or the test above proves nothing. A race, so up to three
/// fresh servers are tried.
#[test]
fn without_serialisable_autocommit_concurrent_increments_are_lost() {
    ...
    panic!(
        "three runs without serialisable autocommit lost nothing — the transaction \
         is not the thing this test proves"
    );
}
